...
public class SecuredUI extends UI {
	...
	protected void init(VaadinRequest request) {
		...
		// Fire up a timer to demonstrate server push. Do NOT use timers in real-world
		// applications, use a thread pool.
		timer = new Timer();
	...
	private void updateTimeAndUser() {
		// Demonstrate that server push works, but the security context is not available
		// inside the Timer thread
		// since it is thread-local and populated by a servlet filter.
		access(() -> timeAndUser
				.setValue(String.format("The server-side time is %s and the authentication token in this thread is %s",
